...
class Indexer
  ...
  def index(doc)
    ...
    yield(words_from(text))
  end

  def words_from(text)
    words = text.gsub(/[^\w\s]/, '').split
    d = []
    words.each { |word| d << word.downcase.stem unless COMMON_WORDS.include?(word) || (word.size > 50) }
    d
  end
end