export async function enter(element, toggle = 'hidden', transitionName = null) {
    element.classList.remove(toggle)
    await transition('enter', element, transitionName)
}

export async function leave(element, toggle = 'hidden', transitionName = null) {
    await transition('leave', element, transitionName)
    element.classList.add(toggle)
}

export async function toggle(element, toggle = 'hidden', transitionName = null) {
    if (element.classList.contains(toggle)) {
        await enter(element, toggle, transitionName)
    } else {
        await leave(element, toggle, transitionName)
    }
}
